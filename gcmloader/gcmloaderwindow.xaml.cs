...
namespace gcmloader
{
    // Heavy secondary UI (“islands”) should load on demand (Loaded / navigate), not in the shell ctor.
...
private static readonly string SettingsFilePath = Path.Combine(SettingsFolder, "settings.toml");
        ...
        public MainWindow()
        {
           // Logger.Initialize();
            ...
            _launcherService = new LauncherService(new LauncherShell
            {
                MakeSelfNonTopmost = MakeSelfNonTopmost,
                GetHostWindowHandle = () => WinRT.Interop.WindowNative.GetWindowHandle(this),
                FindSteamBigPictureWindow = FindSteamBigPictureWindow,
                ForcefullyBringToForeground = ForcefullyBringToForeground,
                ShowMessageAsync = async msg => await messagebox(msg),
                BackToWindows = BackToWindows,
                StartXboxAsync = StartXbox,
            });
...
                        }
        ...
        {
            // No Winlogon / shell swap / UAC — DeckTop is not a shell replacement.
            try
            {
                Logger.Log("BackToWindows — cleanup before exit");

                try { _focusCheckTimer?.Stop(); } catch { }
                try { _minimizeGracePeriodTimer?.Stop(); } catch { }
                try { _mouseIdleTimer?.Stop(); } catch { }
                try { _statusUpdateTimer?.Stop(); } catch { }
                try { _displayWatchdogTimer?.Stop(); } catch { }
                try { _windowEngineTimer?.Stop(); } catch { }
                try { _autoMouseTimer?.Stop(); } catch { }
                try { _taskRefreshTimer?.Stop(); } catch { }
                try { _timer?.Stop(); } catch { }
                try { _refreshTimer?.Stop(); } catch { }

                try { TaskbarManager.RestoreOriginalState(); }
                catch (Exception ex) { Debug.WriteLine($"[Exit] TaskbarManager.RestoreOriginalState: {ex.Message}"); }

                try { TaskbarVisibility.ShowTaskbar(); }
                catch (Exception ex) { Debug.WriteLine($"[Exit] TaskbarVisibility.ShowTaskbar: {ex.Message}"); }

                try { RenameSteamStartupVideo_End(); }
                catch (Exception ex) { Debug.WriteLine($"[Exit] RenameSteamStartupVideo_End: {ex.Message}"); }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[Exit] BackToWindows: {ex.Message}");
            }
            finally
            {
                try { CleanupLogging(); } catch { }
                Environment.Exit(0);
            }
        }
        ...
        public static string? AutoDetectLauncherPath(string launcher) => LauncherService.AutoDetectLauncherPath(launcher);

        private async void SwitchToSpecificLauncher(string launcherId)
        {
            await _launcherService.SwitchToSpecificLauncherAsync(launcherId).ConfigureAwait(true);
        }

        private async void SwitchToConfiguredLauncher()
        {
            await _launcherService.SwitchToConfiguredLauncherAsync().ConfigureAwait(true);
        }
...
        private FocusArea _previousFocusArea = FocusArea.Cards; // Used to track which area to return to from TopButtons
        ...
        private void TransitionToMainUI()
        {
            ...
            DeferredInitialization.RunAfterFirstPaint(DispatcherQueue, () =>
            {
                Debug.WriteLine("[DeferredInit] Post–first paint.");
                GitHubUpdateCheck.RunOptionalFireAndForget();
            });
        }

        // --- STEAM INJECTION ---

        public static void RenameSteamStartupVideo_Start() => LauncherService.RenameSteamStartupVideo_Start();
        ...
        private void LaunchApp(AppInfo app)
        {
            ...
            try
            {
                ...
                {
                    LauncherService.StartProcessAsNonAdmin(app.FilePath);
                }
